...
;(function ( $, window, document, undefined ) {
  ...
  $.extend(Plugin.prototype, {
    ...
    addRow: function() {
      ...
      $newTemplate.find('input').each(function(i, el) {
        el.name = el.name.replace(/\[NEW_ID\]/i, "[" + generatedId + "]");
        el.id = el.id.replace(/NEW_ID/i, generatedId);
      });

      this.$objects.nested_wrapper.append($newTemplate);
    },

    // Remove row
    removeRow: function(element) {
      var $nestedRow = $(element).parent().parent(this.options.row),
